...
extern "C" {
	...
	extern void WriteSLUErrorMessage(char* str);

#ifdef __cplusplus
  }
#endif

 //sprintf_s(msg,"Error: %s at line %d in file %s\n",err_msg,__LINE__, __FILE__);
#define ABORT(err_msg) \
 { char msg[256];\
 sprintf_s(msg,256,"Error: %s",err_msg);\
   WriteSLUErrorMessage(msg); \
   USER_ABORT(msg); }

#define DO_SLU_MESSAGE 0

#define SLU_MESSAGE(err_msg) \
 { char msg[256];\
   sprintf_s(msg,256,"WARNING: %s",err_msg);\
   WriteSLUErrorMessage(msg); }
...
#define CHECK_MALLOC(where) {                 \
    ...
    char str[256]; \
		sprintf_s(str,256,"%s: malloc_total %d Bytes\n",     \
