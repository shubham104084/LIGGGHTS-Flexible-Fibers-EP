...
FixFreeze::FixFreeze(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg)
{
    if (narg != 3)
        error->all(FLERR, "Illegal fix freeze command");
    ...
    force_flag = 0;
    foriginal[0] = foriginal[1] = foriginal[2] = foriginal[3] = foriginal[4] = foriginal[5] = 0.0;
    fix_Ksl_ = NULL;
}
...
void FixFreeze::init()
{
    ...
    for (int i = 0; i < modify->nfix; i++)
        if (strcmp(modify->fix[i]->style, "freeze") == 0)
            count++;
    if (count > 1)
...
void FixFreeze::post_force(int vflag)
{
    ...
    for (int i = 0; i < nlocal; i++)
        if (mask[i] & groupbit)
        {
            ...
            foriginal[1] += f[i][1];
            foriginal[2] += f[i][2];
            foriginal[3] += torque[i][0];
            foriginal[4] += torque[i][1];
            foriginal[5] += torque[i][2];
            f[i][0] = 0.0;
            f[i][1] = 0.0;
            f[i][2] = 0.0;
            torque[i][0] = 0.0;
...
double FixFreeze::compute_vector(int n)
{
    ...
    if (force_flag == 0)
    {
        MPI_Allreduce(foriginal, foriginal_all, 6, MPI_DOUBLE, MPI_SUM, world);
        force_flag = 1;
    }
    if (n < 0 || 5 < n)
        error->all(FLERR, "Value of n not valid: 0 <= n < 3.");
